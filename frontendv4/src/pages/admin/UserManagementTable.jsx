...
const UserManagementTable = ({ users, onRefresh, onSort, renderSortIcon }) => {
    ...
    const handleDeleteUser = async (userId, userName, userRole) => {
        // Chỉ Admin mới được xóa/vô hiệu hóa user
        if (currentUser?.role !== 'Admin') {
            toast.error("Bạn không có quyền vô hiệu hóa tài khoản người dùng.");
            return;
        }
                    ...
                    {users.map((user) => (
                        <tr key={user.id} className="hover:bg-gray-50 transition-colors">
                            ...
                            <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-center space-x-1">
                                ...
                                {/* Chỉ Admin mới được vô hiệu hóa user khác và không thể vô hiệu hóa Admin khác hoặc chính mình */}
